...
def get_atom_fdim(args: Namespace) -> int:
    ...
    if args.atom_features_type == 'base':
        return ATOM_FDIM
    elif args.atom_features_type == 'unimolv1':
        return 512
    elif args.atom_features_type == 'unimolv2':
        return 768
    else:
        raise ValueError(f'Do not have atom_features_type of {args.atom_features_type}')
...
class MolGraph:
    ...
    def __init__(self, smiles: str, args: Namespace, prompt: bool, func_repr=None):
        """
        ...
        for a in mol.GetAtoms():
            if a.GetAtomicNum() == 1 and a.GetIsotope() > 0:
                ...
                a.SetIsotope(0)
        if flag_isotopes:
            # 只要 SMILES 里出现了 [2H] 这种显式氢，RemoveHs 才会真正删掉它
        ...
        if not self.prompt:
            ...
            elif args.atom_features_type == 'unimolv1' or args.atom_features_type == 'unimolv2':
                self.f_atoms = list(func_repr(smiles))
            else:
                raise ValueError(f'Do not have atom_features_type of {args.atom_features_type}')

            self.f_atoms = [list(self.f_atoms[i]) for i in range(self.n_atoms)]
            ...
            for i, atom in enumerate(mol.GetAtoms()):
                ...
                self.f_atoms.append(self.atom_features[i, :])
...
def mol2graph(smiles_batch: List[str],
              args: Namespace, prompt: bool, func_repr=None) -> BatchMolGraph:
    """
    ...
    for smiles in smiles_batch:
        ...
        # else:
        mol_graph = MolGraph(smiles, args, prompt, func_repr)
