...
if __name__ == '__main__':
    ...
    for name in ['OH', 'SO4-', 'O3', '1O2', 'Fe(VI)', 'HClO']:
        train_data = pd.read_csv(f'./data/{name}-train-{0}.csv')['0']
        test_data = pd.read_csv(f'./data/{name}-test-{0}.csv')['0']
        all_molecule_smiles += list(train_data)
        all_molecule_smiles += list(test_data)
    all_molecule_smiles = list(set(all_molecule_smiles))
    data = {
        '0': all_molecule_smiles,
        '1': ['0' for _ in range(len(all_molecule_smiles))],
    }
    df = pd.DataFrame(data)
    df.to_csv('./data/all_smiles.csv', index=False)

    '''get GNN embeddings for molecule in dataset'''
    os.makedirs('./Result/infos', exist_ok=True)
    ...
    for name in name_list:
        for seed in [0, 1, 2, 3, 4]:
            ...
            all_data_path = f'./data/all_smiles.csv'
            typical_data_path = f'./data/typical.csv'
            train_dataset = get_data(path=train_data_path, args=args)
            test_dataset = get_data(path=test_data_path, args=args)
            all_dataset = get_data(path=all_data_path, args=args)
            typical_dataset = get_data(path=typical_data_path, args=args)
            ...
            if args.atom_features_type == 'base':
                ...
                mappins_smiles_to_embedding = get_smi_to_embedding_map('all', args.atom_features_type)
                emb = []
            ...
            all_smiles, all_embeddings, all_labels, all_preds = get_embeddings(model, all_dataset, 128)
            typical_smiles, typical_embeddings, typical_labels, typical_preds = get_embeddings(model, typical_dataset, 128)
            ...
            torch.save(all_data, f"Result/infos/all-{name}-{seed}.pt")

            typical_data = {
                "names": typical_smiles,
                "labels": typical_labels,
                "preds": typical_preds,
                "embeddings": list(typical_embeddings),
            }
            torch.save(typical_data, f"Result/infos/typical-{name}-{seed}.pt")